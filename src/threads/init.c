...
/*
static struct list test_list; //test purpose
...
static void add_member() {
  struct test_elem* test;
  test -> a = 1;

  list_push_front(&test_list, &test -> elem);
}
*/
/* Pintos main program. */
int
main (void)
{
  ...
  /* Initialize interrupt handl
  ers. */
  intr_init ();//init IDT for 0-255
  timer_init ();//for both tick counting and thread switching
  kbd_init ();//8042 Keyboard init
  input_init ();
#ifdef USERPROG
  exception_init ();
  syscall_init ();

...
/*handlers in idt are always element of array intr_stubs*/
...
intr_init (void)
{
  ...
  /* Initialize IDT. */
  for (i = 0; i < INTR_CNT; i++)//traverse from 0-255
    idt[i] = make_intr_gate (intr_stubs[i], 0);
    //interrupt gate will clear IF in eflags
    //so initially all gates in idt is interrupt gates
  /* Load IDT register.
                  ...
                  intr_handler_func *handler, const char *name)
{
  ...
  //interrupt gates clear the IF bit in EFLAGS, while trap gates do not.
  if (level == INTR_ON)
    idt[vec_no] = make_trap_gate (intr_stubs[vec_no], dpl);
  else
    idt[vec_no] = make_intr_gate (intr_stubs[vec_no], dpl);
  intr_handlers[vec_no] = handler;
  //if this not called yet, intr_handler[xxx] is NULL
  //handler will be fetched called in intr_handler
...
intr_handler (struct intr_frame *frame)
{
  ...
  if (external)
    {
      ...
      //interrupt gates clear the IF bit in EFLAGS
      ASSERT (!intr_context ());
  ...
  if (handler != NULL)
    handler (frame);
    /*when int20,
    which is time interrupt,
    this will call intr_yield_on_return
    so finnally thread will switch*/
  ...
  if (external)
    {
      ...
      if (yield_on_return)
        thread_yield ();
      //switch threads when 0x20 and is multiple of TIME_SLICE
    }
}

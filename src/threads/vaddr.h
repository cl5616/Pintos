...
static inline uintptr_t entry_to_addr(uint32_t entry)
{
  return (entry & ~PGMASK);
}

static inline
void clear_p(uint32_t* page_entry)
{
  asm volatile ("mov %0, %%eax; and $0xfffffffe, (%%eax)" : : "m" (page_entry));
}

static inline
void set_p(uint32_t* page_entry)
{
  asm volatile ("mov %0, %%eax; or $0x1, (%%eax)" : : "m" (page_entry));
}

static inline
void set_addr(uint32_t* page_entry, uintptr_t paddr)
{
  ASSERT((paddr & 0xfff) == 0);
  asm volatile ("mov %0, %%eax; andl $0xfff, (%%eax);\
                mov %1, %%ecx; or %%ecx, (%%eax)"
                : : "m" (page_entry), "m" (paddr));
}
